...
export async function deleteLaporan(id: number) {
  ...
  await prisma.$transaction([
    prisma.bbm_log.deleteMany({ where: { laporan_id: id } }),
    prisma.pengeluaran.deleteMany({ where: { laporan_id: id } }),
    prisma.setoran.deleteMany({ where: { laporan_id: id } }),
    prisma.laporan_harian.delete({ where: { id } }),
  ]);
